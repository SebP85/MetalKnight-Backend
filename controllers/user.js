      ...
      if(!user){//Utilisateur non trouvé
        ...
        if(!user.userConfirmed){//utilisateur non validé
          if(process.env.DEVELOP === "true") {
            ...
            console.log('---------------------------------------------------------    Requête erreur    ------------------------------------------------------------------');
            res.status(config.erreurServer.BAD_REQUEST).json({ error: 'Utilisateur pas encore confirmé dans la BDD !' });
          } else {
              ...
              User.updateOne({ email: user.email }, user)
                .then(() => {
                  ...
                  if(process.env.DEVELOP === "true") {
                    ...
                    console.log('---------------------------------------------------------    Requête erreur    ------------------------------------------------------------------');
                    res.status(config.erreurServer.BAD_REQUEST).json({ error: 'nombre de tentatives de connexion max atteint' });
                  } else {
                    ...
                    User.updateOne({ email: user.email }, user)
                      .then(() => {
                        ...
                        if(process.env.DEVELOP === "true") {
                          console.log('Mot de passe incorrect !');
                          console.log('---------------------------------------------------------    Requête erreur    ------------------------------------------------------------------');
                          res.status(config.erreurServer.BAD_REQUEST).json({ error: 'Mot de passe incorrect !' });
                        } else {
                          logger.error('Mot de passe incorrect !');
                          res.status(config.erreurServer.BAD_REQUEST).json({ error: process.env.MSG_ERROR_PRODUCTION });
                        }
...
exports.verifyRecaptcha = (req, res, next) => {//On vérifie le token recaptcha
  ...
  if (!req.body.tokenRecaptcha) {//Présence du token ?
    if(process.env.DEVELOP === "true") {
      ...
      console.log('---------------------------------------------------------    Requête erreur    ------------------------------------------------------------------');
      res.status(config.erreurServer.BAD_REQUEST).json({message: "recaptchaToken is required"});
    } else {
    ...
    .then((response) => {
      ...
      if(response.data.success){
        ...
        if(process.env.DEVELOP === "true") {
          ...
          console.log('---------------------------------------------------------    Requête erreur    ------------------------------------------------------------------');
          res.status(config.erreurServer.BAD_REQUEST).json({message: "recaptchaToken n'est pas le bon"});
        } else {
